...
import openai
import google.generativeai as palm  # Correctly import google generative ai
...
openai_api_key = "sk-proj-J1cheCMS20UIB8EFH4bvT3BlbkFJcBoPSeUSi1NUp6jBsVyO"
    ...
    try:
        response = palm.generate_text(
            model='models/text-bison-001',  # Update with the appropriate model ID
                ...
                prompt= f"you are an helpful assistant and need to give the response in the language:{selected_language}.{prompt}",  # Corrected variable usage
...
def toggle_api():
    if st.session_state.selected_api == 'OpenAI':
        st.session_state.selected_api = 'Google Gemini'
    else:
        st.session_state.selected_api = 'OpenAI'

# Place the toggle button in the top left
col1, col2 = st.columns([9, 9])
with col1:
    st.button('Toggle API', on_click=toggle_api)
with col2:
